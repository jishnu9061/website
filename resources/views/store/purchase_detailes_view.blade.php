        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <tr>
                    ...
                    <td style="width:20%"><select class="form-control" aria-label="Default select example"
                            name="manufacturer" disabled>
                            <option value="">Select</option>
                            @foreach($manufact as $key)
                            <option value="{{$key->id}}" {{($key->id == $data->id) ? "selected" : "" }}>
                                {{$key->manufactuers_name}}</option>
                            @endforeach
                        </select></td>
                    <td style="width:20%"> <input type="date" name="purchase_date" value="{{$data->purchase_date}}"
                            class="form-control" readonly></td>
                    <td style="width:20%"><input type="text" name="purchase_number" value="{{$data->purchase_orderno}}"
                            class="form-control" style="pointer-events: none;" readonly></td>
                </tr>
                <tr>
                    <thead>
                        <tr>
                ...
                <tr>
                    ...
                    <td scope="col"><select class="form-select" aria-label="Default select example" name="payment_type"
                            ...
                            <option value="{{$key1->payment_id}}"
                                {{($key1->payment_id == $data->payment_id) ? "selected" : "" }}> {{$key1->payment_name}}
                            </option>
                            @endforeach
                        </select></td>
                    <td scope="col"><select class="form-select" aria-label="Default select example" name="supplier"
                            ...
                            <option value="{{$key->id}}" {{ ($key->id == $data->id) ? "selected" : "" }}>
                                {{$key->supplier_name}}</option>
                            @endforeach
                        </select></td>
                    <td scope="col"><select class="form-select" aria-label="Default select example" name="status"
                            disabled>
                            <option value="Order Placed" @if ($data->status == "OrderPlaced")selected="selected"
                                @endif>Order Placed</option>
                ...
                <tr>
                    <td>
                        <select class="form-select" aria-label="select example" name="item_name[]" id="item_name"
                            ...
                            <option value="{{$val->id}}" {{ ($val->id == $row->item_name) ? "selected" : "" }}>
                                {{ $val->Item_name }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td><input type="text" name="batch_no[]" id="batch_no" class="form-control"
                            value="{{$row->batch_no}}" disabled></td>
                    <td><input type="text" name="quantity[]" id="quantity" class="form-control  calculate"
                            value="{{$row->quantity}}" disabled></td>
                    <td><input type="text" name="price[]" id="price" class="form-control calculate"
                            value="{{$row->price}}" disabled></td>
                    <td><input type="text" name="total[]" id="total" class="form-control calculate-sub"
                            value="{{$row->total}}" disabled></td>
                </tr>
                @endforeach
    ...
    <div class="col-sm">
        <table class="table table-bordered">
            <tr>
                ...
                <td><input type="text" class="form-control invoice-sub-total" value="" id="exampleFormControlInput1"
                        name="grand_totall" id="grand-total" readonly></td>
            </tr>
            <tr>
                <td scope="col">Advance Amount</td>
                <td><input type="text" class="form-control" id="exampleFormControlInput1" name="advance_amount"
                        value="{{$data->advance_amount}}" id="advance-amount" readonly></td>
            </tr>
            <tr>
                <td scope="col">Pending Amount</td>
                <td><input type="text" class="form-control" name="pending_amount"
                        value="<?php echo ($data->grand_total)-($data->advance_amount);?>" id="result" readonly></td>
            </tr>
            {{-- <tr>
                                            <td scope="col">Repayment Amount</td>
                                            <td><input type="text" class="form-control" name="pending_amount" value="{{0}}"
            id="result" readonly></td>
            </tr>
            <tr>
                <td scope="col">Repayment Date</td>
                <td><input type="date" class="form-control" name="pending_amount"
                        value="<?php echo ($data->grand_total)-($data->advance_amount);?>" id="result" readonly></td>
            </tr> --}}
