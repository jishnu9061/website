...
<table class="table table-hover" id="allpatients" style="margin-top: 40px;">
  ...
  <tbody>
    ...
    <tr>
      ...
      <th>{{$test->tests}}</th>
      <form method="post" action="submitlabres">
        @csrf
        <input type="hidden" name="hiddenid" value="{{$test->id}}">
      <td><input type="text" name="observed" class="form-control"></td>
      <th scope="col">Unit</th>
      <th scope="col">{{$test->reference_range}}</th>
      <td><button type="submit" class="form-control btn btn-primary">Submit</button></td>
      </form>
      <td></td>
    </tr>
