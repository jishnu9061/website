             ...
             <tr >
                ...
                <select class="custom-select2 form-control select2-hidden-accessible select2_function" onchange="check_select_values(this.value)"  style="width: 100%; height: 38px;" id="stock-item-select">
                    ...
                    {{-- @foreach($items as $acc)
                     <option value="{{ $acc->id }}">  {{ $acc->stock_items_itemcode."--".$acc->stock_items_name." [ ".$acc->stock_categories_name."--".$acc->stock_subcategories_name." ] " }} </option>
                    @endforeach; --}}
...
if ($("#journal_form")[0].checkValidity()){
        ...
        $.ajaxSetup({
            ...
            $.ajax({
            url:"{{ route('create_new_stock_transaction') }}",
            type:"POST",
            data: {
