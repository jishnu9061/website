...
<div class="container-fluid">
    <div class="row">
      ...
      print_r($journal_transaction);

      @endphp  --}}
        <div class="col-md-3">
            <div class="form-group mb-3">
                <input type="date" value="{{ date('Y-m-d',strtotime($journal_transaction->journal_date)) }}" class="form-control" name="journal_date" id="journal-date"  required>
            </div>
@csrf
        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <table class="table table-striped" id="update_credit_table">
                ...
                @foreach ($journal_items as $journal)
                @if($journal->journal_items_type==2)
                <tr>
                    <td>
                        <select class="form-control"  name="edit_credit_company[]" >
                            <option value=""> Select Account</option>
                            @foreach($account as $acc)
                             <option value="{{ $acc->id }}" {{ ($acc->id == $journal->journal_transaction_account) ? "selected" : "" }} > {{ $acc->accounts_name }}</option>
                            @endforeach;
                    ...
                    <td>
                        <input type="text" onchange="total_credit_amount()" name="edit_credit_amount[]"  value="{{ $journal->journal_items_amount }}" class="form-control credit_amount_update">

                    </td>
                    <td>
                        <input type="hidden" name="edit_credit_trans_item[]" value="{{ $journal->journal_id }}">

                        <button  type="button"  class="form-control btn-danger delete"  value="{{ $journal->journal_id }}" ><i class="bi bi-trash"></i></button>
                    </td>
                </tr>
                @endif
                @endforeach
        ...
        <div class="col-md-6">
            <table class="table table-striped" id="update_debit_table">
                ...
                @foreach ($journal_items as $journal)
                @if($journal->journal_items_type==1)
                <tr>
                    <td>
                    ...
                    <td>
                        <input type="text" name="edit_debit_amount[]"  onchange="total_debit_amount()" value="{{ $journal->journal_items_amount }}" class="form-control debit_amount_update">

                    </td>
                    <td>
                        <input type="hidden" name="edit_debit_trans_item[]" value="{{ $journal->journal_id }}">
                        <button  type="button" class="form-control btn-danger delete" value="{{ $journal->journal_id }}" ><i class="bi bi-trash"></i></button>
                    </td>
                </tr>
                @endif
   ...
   <hr/>
    ...
    <div class="row">
        ...
        <table class="table table-striped table">
            <tr>
                <th>Narration</th>
                <th><input type="text"  value="{{ $journal_transaction->journal_narration }}" name="journal_narration" id="journal-narration"  class="form-control" required style="height:50px;"></th>
            </tr>
            <tr>
                <th colspan="1">
...
//FOR THE DEBIT SECTION
 function add_debit(){
     ...
     htmlRows += '<td><select class="form-control" required  name="debit_from_account[]" id="debit_from_account_'+count+'" >';
        ...
        <?php endforeach; ?>
         htmlRows += '</select></td>';
         {{--  htmlRows += '<td><select class="form-control" required  name="debit_to_account[]" id="debit_to_account_'+count+'" >';
            <?php foreach($account as $key): ?>
            htmlRows +='<option value="<?php echo $key->id  ?>"><?php echo ucfirst($key->accounts_name); ?></option>';
            <?php endforeach; ?>
             htmlRows += '</select></td>';  --}}
...
function add_credit(){
    ...
    var htmlRows = '';
        ...
        {{--  htmlRows += '<td><select class="form-control" required name="credit_from_account[]" id="credit_from_account_'+count+'" >';
       <?php foreach($account as $key): ?>
       htmlRows +='<option value="<?php echo $key->id  ?>"><?php echo ucfirst($key->accounts_name); ?></option>';
       <?php endforeach; ?>
        htmlRows += '</select></td>';  --}}
            ...
            if (willDelete) {
                ...
                $.ajaxSetup({
                    ...
                    data: {
                        ...
                        transaction_id:{{$journal_transaction->transaction_id}},
                    },
